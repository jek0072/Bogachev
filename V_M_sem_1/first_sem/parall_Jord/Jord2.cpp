...
//		printf("%f\n",*inform->N);
    for(int i=0;i<k;i++){
       ...
       if(res>=0){
        ...
        pthread_barrier_wait(inform->barrier);//DON'T WORK without it
    ...
    if(id==0){
		...
		double*T1=new double[p*p];
		 if(p!=0){
		    if (inverse_mat(A+k*(d*d*k+p*d)+p*d*k,IN,p,*inform->N)==0){
